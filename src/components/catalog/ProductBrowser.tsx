...
export function ProductBrowser({
  ...
  // id → name + main → sub ids (for scoping + product eyebrows)
  ...
  // Build spec dropdown filters from the scoped products (keys with ≥2 values).
  ...
  const filtered = useMemo(() => {
    // Word-based, multi-field search: every typed word must appear somewhere in
    // the product's name, category, description or specs (order-independent).
    ...
    return list;
  }, [scoped, query, filters, sort, catName]);
    ...
    <div className="space-y-6 rounded-3xl border border-ink/10 bg-white p-5 shadow-[0_18px_50px_-32px_rgba(20,20,15,0.35)]">
      ...
      {specFilters.length > 0 ? (
        specFilters.map(({ key, values }) => (
          <div key={key}>
            <label className="block text-sm font-semibold text-ink">{key}</label>
            <div className="relative mt-2">
              <select
                value={filters[key] ?? ""}
                onChange={(e) => setFilter(key, e.target.value)}
                className={cn(
                  "w-full appearance-none rounded-full border bg-white py-3 pl-4 pr-10 text-sm outline-none transition-colors focus:border-ink/40",
                  filters[key] ? "border-ink/40 text-ink" : "border-ink/15 text-ink/55",
                )}
              >
        ...
        <p className="text-sm text-ink/45">No filters available for this range.</p>
      )}
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[16rem_1fr] lg:gap-12">
      ...
      <aside className="lg:sticky lg:top-36 lg:self-start">
        ...
        <div className={cn(open ? "block" : "hidden lg:block")}>{FilterPanel}</div>
      </aside>
          ...
          <p className="text-sm text-ink/55">
            {total === 0 ? (
              ...
              <>
                ...
                of <span className="font-semibold text-ink">{total}</span> results
              </>
            )}
          </p>
        ...
        {pageItems.length > 0 ? (
          ...
          <div className="rounded-3xl border border-ink/10 bg-paper-2 p-12 text-center">
            <p className="text-ink/60">
              No products match your filters{query ? ` / “${query.trim()}”` : ""}.
            </p>
            <button
              type="button"
              onClick={clearAll}
